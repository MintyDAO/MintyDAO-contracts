...
interface IFormula {
  function computeRewards() external view returns(uint);
}
...
contract VotersRewardsLock is Ownable {
  ...
  function computeRewards() public view returns(uint){
    return IFormula(formula).computeRewards();
  }
  ...
  // allow destribute rewards once a week
  function destributeRewards() external {
    require(formula != address(0), "Zerro formula");
