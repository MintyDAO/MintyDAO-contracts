...
contract yMetaPool is Ownable, ReentrancyGuard {
    ...
    function compoundDividend() external payable nonReentrant {
        ...
        if (pending > 0) {
            ...
            if (address(stakingToken) != address(dividendToken)) {
                if (address(dividendToken) == address(0x0)) {
                    address wethAddress = IPancakeRouter02(uniRouterAddress)
                        .WETH();
    ...
    function _resetAllowances() internal {
        ...
        if (address(dividendToken) == address(0x0)) {
            address wethAddress = IPancakeRouter02(uniRouterAddress).WETH();
