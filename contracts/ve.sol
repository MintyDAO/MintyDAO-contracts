...
contract ve is IERC721, IERC721Metadata {
    ...
    uint256 internal constant MAXTIME = 16 * 7 * 86400;
    int128 internal constant iMAXTIME = 16 * 7 * 86400;
    ...
    mapping(uint256 => uint256) internal idPerDeadLine;
    ...
    function _deposit_for(
        ...
        _locked.lockedtime = block.timestamp;
        if (unlock_time != 0) {
    ...
    function abstain(uint256 _tokenId) external {
        ...
        require(
            block.timestamp > idPerDeadLine[_tokenId],
    ...
    function withdraw(uint256 _tokenId) external nonreentrant {
        ...
        if (_locked.end - _locked.lockedtime >= 16 weeks) {
            assert(
                IERC20(token).transfer(
                    msg.sender,
                    (uint256(int256(_locked.amount)) * 25) / 100
                )
            );
        }
    ...
    function setMaxX(uint256 _x) external {
